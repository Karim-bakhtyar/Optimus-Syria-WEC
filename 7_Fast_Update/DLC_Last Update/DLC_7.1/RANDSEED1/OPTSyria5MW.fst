...
---------------------- SIMULATION CONTROL --------------------------------------
    ...
    "FATAL"   AbortLevel      - Error level when simulation should abort (string) {"WARNING", "SEVERE", "FATAL"}
     ...
     9999.9   DT_UJac         - Time between calls to get Jacobians (s)
...
"OPTSyria5MW_ServoDyn_FBFF.dat" ServoFile - Name of file containing control and electrical-drive input parameters (quoted string)
...
----------------------- LINEARIZATION ------------------------------------------ 
     ...
     9999.9   TrimCase        - Controller parameter to be trimmed {1:yaw; 2:torque; 3:pitch} [used only if CalcSteady=True] (-)
     9999.9   TrimTol         - Tolerance for the rotational speed convergence [used only if CalcSteady=True] (-)
     9999.9   TrimGain        - Proportional gain for the rotational speed error (>0) [used only if CalcSteady=True] (rad/(rad/s) for yaw or pitch; Nm/(rad/s) for torque)
     9999.9   Twr_Kdmp        - Damping factor for the tower [used only if CalcSteady=True] (N/(m/s))
     9999.9   Bld_Kdmp        - Damping factor for the blades [used only if CalcSteady=True] (N/(m/s))
     9999.9   NLinTimes       - Number of times to linearize (-) [>=1] [unused if Linearize=False]
     9999.9   LinTimes        - List of times at which to linearize (s) [1 to NLinTimes] [used only when Linearize=True and CalcSteady=False]
     9999.9   LinInputs       - Inputs included in linearization (switch) {0=none; 1=standard; 2=all module inputs (debug)} [unused if Linearize=False]
     9999.9   LinOutputs      - Outputs included in linearization (switch) {0=none; 1=from OutList(s); 2=all module outputs (debug)} [unused if Linearize=False]
...
---------------------- VISUALIZATION ------------------------------------------
        ...
        9999.9   VTK_fps      - Frame rate for VTK output (frames per second){will use closest integer multiple of DT} [used only if WrVTK=2 or WrVTK=3]
